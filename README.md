...
XInputDeviceListener listener = new SimpleXInputDeviceListener() {
    ...
    public void disconnected() {
        // Pause the game and display a message暂停游戏并显示消息
    }

    @Override
    public void buttonChanged(final XInputButton button, final boolean pressed) {
        // 给定的按钮刚刚按下（如果按下 == true）或释放（按下 == false）}The given button was just pressed (if pressed == true) or released (pressed == false)
    }
};
//每当轮询设备时，只要有更改，就会触发侦听器事件
...
```java
XInputDevice device = ...;
//从 0 到 65535 的振动速度，其中 0 = 无振动，65535 = 最大振动 
// 超出范围的值引发 IllegalArgumentException
